...
import com.fastdfs.common.IniFileReader;
...
public class ClientGlobal {
	...
	public static String httpAccessUrl;
	...
	public static void init(InputStream is) throws Exception {
		...
		iniReader = new IniFileReader(is);

		g_connect_timeout = iniReader.getIntValue("connect_timeout", DEFAULT_CONNECT_TIMEOUT);
		if (g_connect_timeout < 0) {
			g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
		}
		httpAccessUrl = iniReader.getStrValue("http.access.url");
		if (!httpAccessUrl.endsWith("/"))
			httpAccessUrl += "/";
		...
		if (szTrackerServers == null) {
			throw new CustomException("item \"tracker_server\" in " + is + " is empty");
		}
	...
	public static void init(String file) throws Exception {
		...
		iniReader = new IniFileReader(file);
		...
		szTrackerServers = iniReader.getValues("tracker_server");
		if (szTrackerServers == null) {
			throw new CustomException("item \"tracker_server\" in " + file + " 没有找到");
		}

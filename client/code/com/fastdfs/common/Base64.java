 ...
 * loosely based on the Base64 Reader by Dr. Mark Thornton Optrak Distribution
 * Software Ltd. http://www.optrak.co.uk and Kevin Kelley's
 ...
 * Java.net.URLEncoder.encode/decode If you don't like this code, there is
 ...
 * simpler to code, but not as compact.
 *
 ...
 * differences.
 *
 * version 1.4 2002 February 15 -- correct bugs with uneven line lengths, allow
 * you to configure line separator. now need Base64 object and instance methods.
 * new mailing address. version 1.3 2000 September 12 -- fix problems with
 * estimating output length in encode version 1.2 2000 September 09 -- now
 * handles decode as well. version 1.1 1999 December 04 -- more symmetrical
 * encoding algorithm. more accurate StringBuffer allocation size. version 1.0
 * 1999 December 03 -- posted in comp.lang.java.programmer. Futures Streams or
 * files.
 */
...
public class Base64 {
	...
	public static void main(String[] args) {
		...
		if (debug) {
			...
			/*
			 * byte[] a = { (byte)0xfc, (byte)0x0f, (byte)0xc0}; byte[] b = {
			 * (byte)0x03, (byte)0xf0, (byte)0x3f}; byte[] c = { (byte)0x00,
			 * (byte)0x00, (byte)0x00}; byte[] d = { (byte)0xff, (byte)0xff,
			 * (byte)0xff}; byte[] e = { (byte)0xfc, (byte)0x0f, (byte)0xc0,
			 * (byte)1}; byte[] f = { (byte)0xfc, (byte)0x0f, (byte)0xc0,
			 * (byte)1, (byte)2}; byte[] g = { (byte)0xfc, (byte)0x0f,
			 * (byte)0xc0, (byte)1, (byte)2, (byte)3}; byte[] h =
			 * "AAAAAAAAAAB".getBytes();
			 * 
			 * 
			 * 
			 * show(a); show(b); show(c); show(d); show(e); show(f); show(g);
			 * show(h); Base64 b64 = new Base64();
			 * show(b64.decode(b64.encode(a))); show(b64.decode(b64.encode(b)));
			 * show(b64.decode(b64.encode(c))); show(b64.decode(b64.encode(d)));
			 * show(b64.decode(b64.encode(e))); show(b64.decode(b64.encode(f)));
			 * show(b64.decode(b64.encode(g))); show(b64.decode(b64.encode(h)));
			 * b64.setLineLength(8); show((b64.encode(h)).getBytes());
			 */
		}
	}// end main
